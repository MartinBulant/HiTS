...
class Environment:
    def __init__(
        ...
        # Set dimensions and ranges of states, actions, and goals
    ...
    def reset_sim(self, next_goal=None):
        ...
        if self.name == "ant_reacher.xml":
            while True:
                ...
                min_dist = 8
                if np.linalg.norm(next_goal[:2] - self.data.qpos[:2]) > min_dist:
                    break
            ...
            self.data.qpos[0] = np.random.uniform(3, 6.5)
            self.data.qpos[1] = np.random.uniform(3, 6.5)
    ...
    def display_end_goal(self, end_goal):
        ...
        elif self.name in ("ant_reacher.xml", "ant_four_rooms.xml"):
            self.data.mocap_pos[0][:3] = np.copy(end_goal[:3])
    ...
    def display_subgoals(self, subgoals):
        ...
        for i in range(1, min(len(subgoals), 11)):
            if self.name == "pendulum.xml":
                ...
                self.data.mocap_pos[i][:3] = np.copy(subgoals[subgoal_ind][:3])
                self.model.site_rgba[i][3] = 1
                subgoal_ind += 1
