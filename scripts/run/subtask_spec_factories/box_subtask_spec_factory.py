...
from graph_rl.subtasks import (
    BoxInfoHidingTGSubtaskSpec,
    BoxInfoHidingSPSubtaskSpec,
    EnvSPSubtaskSpec,
    EnvRMSubtaskSpec,
)
...
class BoxSubtaskSpecFactory(SubtaskSpecFactory):
    ...
    def get_hits_subtask_spec(cls, env, subtask_spec_params, level, delta_t_max):
        ...
        partial_obs_indices, goal_indices, factorization = (
            cls.get_indices_and_factorization(
                env=env, subtask_spec_params=subtask_spec_params, level=level
            )
        )
        ...
        subtask_specs = subtask_spec_class(
            ...
            goal_achievement_threshold=subtask_spec_params[
                "goal_achievement_threshold"
            ],
            partial_obs_indices=partial_obs_indices,
            goal_indices=goal_indices,
            factorization=factorization,
            delta_t_min=subtask_spec_params.get("delta_t_min", 0.0),
            env=env,
        )
        return subtask_specs

    @classmethod
    def get_hits_subtask_specs(
        cls, env: gymnasium.Env, n_layers, subtask_spec_params_list
    ):
        ...
        delta_t_max_list = [
            spec_params["delta_t_max"] for spec_params in subtask_spec_params_list[:-1]
        ]

        # calculate delta_t_max on second highest level if not specified
        if delta_t_max_list[-1] == -1:
            delta_t_max_list[-1] = (
                max_episode_length / subtask_spec_params_list[-1]["max_n_actions"]
            )
        ...
        for delta_t_max, spec_params, l in zipped_lists:
            subtask_specs.append(
                cls.get_hits_subtask_spec(env, spec_params, l, delta_t_max)
            )
    ...
    def get_hac_subtask_specs(cls, env, n_layers, subtask_spec_params_list):
        ...
        max_n_actions_list = [
            spec_params["max_n_actions"] for spec_params in subtask_spec_params_list
        ]

        # calculate max_n_actions on lowest level if not specified
        if max_n_actions_list[0] == -1:
            max_n_actions_list[0] = env.max_episode_length / np.prod(
                max_n_actions_list[1:]
            )

        subtask_specs = []
        zipped_lists = zip(
            max_n_actions_list[:-1], subtask_spec_params_list, range(n_layers)
        )
        # levels zero to n_layers - 1
        for max_n_act, spec_params, l in zipped_lists:
            subtask_specs.append(
                cls.get_hac_subtask_spec(env, spec_params, l, max_n_act)
            )
